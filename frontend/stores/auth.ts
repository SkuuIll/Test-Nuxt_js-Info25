...
export const useAuthStore = defineStore('auth', () => {
  ...
  if (process.client) {
    api = useApi()
    try {
      const { authLoading: loading } = useLoading()
      authLoading = loading
    } catch (e) {
      ...
      authLoading = {
        ...
        withLoading: async (fn: () => Promise<any>) => {
          authLoading.loading.value = true
          try {
            return await fn()
          } finally {
            authLoading.loading.value = false
          }
        }
      }
    }
    ...
    try {
      ...
      errorHandlers = { handleLoginError, handleRegistrationError, handleTokenError }
    } catch (e) {
      // Fallback error handlers
      errorHandlers = {
        handleLoginError: (err: any) => ({ message: err.message || 'Login failed' }),
        handleRegistrationError: (err: any) => ({ message: err.message || 'Registration failed' }),
        handleTokenError: (err: any) => ({ message: err.message || 'Token error' })
      }
    }
  }
  ...
  const clearAuthState = async () => {
    ...
    if (process.client && api?.tokenUtils) {
